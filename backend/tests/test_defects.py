...
class TestGetDefects:
    ...
    def test_filter_by_equipment(self, client, auth_headers_viewer, defect):
        ...
        assert response.status_code == 200
        assert all(d["equipment_id"] == defect.equipment_id for d in response.json())
...
class TestGetDefectById:
    def test_get_existing(self, client, auth_headers_viewer, defect):
        response = client.get(f"/defects/{defect.id}", headers=auth_headers_viewer)
        assert response.status_code == 200
...
class TestCreateDefect:
    def test_engineer_can_create(self, client, auth_headers_engineer,
                                   equipment, defect_status, defect_type, criticality):
        response = client.post("/defects/", headers=auth_headers_engineer, json={
            "title": "Новый дефект",
...
class TestDeleteDefect:
    def test_admin_can_delete(self, client, db, auth_headers_admin,
        ...
        response = client.delete(f"/defects/{d.id}", headers=auth_headers_admin)
        assert response.status_code == 204

    def test_engineer_cannot_delete(self, client, auth_headers_engineer, defect):
